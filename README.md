# FCM Hosts Optimizer (v3.0)

通过硬链接 (Hard Link) 与原生挂载技术实现的自适应 Systemless Hosts 模块。完美兼容 Magisk, APatch 与 KernelSU。

## ✨ 核心特性

- **高兼容性**: 采用“硬链接 + 管理器原生挂载”方案，彻底解决在非 OverlayFS 环境（或命名空间隔离）下的挂载失效问题。
- **动态更新**: 即使由管理器托管挂载，得益于 Inode 共享技术，IP 同步依然实时生效，无需重启。
- **双语日志**: `service.sh` 与 `fcm-update` 全面支持中英双语日志输出，易于排查问题。
- **低能耗调度**: 由 [MiceTimer](https://github.com/Mice-Tailor-Infra/micetimer) 引擎统一托管，支持 WakeLock 原子写入，确保在深度睡眠下也能稳定更新。
- **安全性**: 强制保留 localhost 定义，自动维持 SELinux 安全上下文。

> [!WARNING]
> **兼容性提示 (养蛊预警)**: 由于本模块采用强制挂载接管策略，安装后会覆盖其他 Hosts 模块的生效规则。建议不要同时安装多个 Hosts 类模块以免产生不可预期的冲突。

## 📥 安装与迁移

1. 确保已安装配套的 [MiceTimer](https://github.com/Mice-Tailor-Infra/micetimer) 模块。
2. 通过 Magisk / KernelSU / APatch 安装本模块并**重启**。
3. **迁移提示**: 若从 v2.x 升级，安装脚本会自动处理旧版冲突，建议安装后检查 `/data/adb/fcm-hosts/service.log`。

## 🛠 常用命令

```bash
# 手动强制更新 FCM 库
/system/bin/fcm-update

# 自测 FCM 连通性
/system/bin/fcm-test
```

## ⚙️ 配置说明

- **数据区**: `/data/adb/fcm-hosts/`
- **定时配置**: `/data/adb/timers.d/fcm-hosts.toml`
- **远程源**: `miceworld.top/fcm-hosts-next/fcm_dual.hosts`

## 📖 技术原理 (Technical Principles)

### 硬链接挂载 (Hard Link Mounting)
v3.0 核心在于安装时执行 `ln -f /data/adb/fcm-hosts/hosts $MODPATH/system/etc/hosts`：
- **Inode 共享**: 两个路径指向磁盘上同一个数据块。
- **权限穿透**: 普通 App 通过路径可见性更高的 `/system` 身份访问，绕过了 `/data/adb` 的权限限制。
- **原生治理**: 让管理器在 Boot 阶段处理挂载，比 `service.sh` 手动挂载更鲁棒。

### 原子原地写入 (In-place Write)
更新脚本使用 `cat >` 覆盖内容而避免 `mv` 动作，从而保证文件 Inode 不变，使所有现有的挂载点内容同步更新。

## ❤️ 致谢

- [FCM Hosts Project](https://miceworld.top/fcm-hosts-next/)
- [Topjohnwu (Magisk)](https://github.com/topjohnwu)
- [Tiann (KernelSU)](https://github.com/tiann)
